...
router.get("/scanner/lookup/:barcode", requireAuth, async (req, res) => {
  ...
  if (result.type === "general") {
    const product = result.product;
    const score = calculateGeneralScore(product, excludedIngredients);
    ...
    console.log(`[Scanner] Saved general barcode=${barcode} name=${product.productName} score=${score.total}`);
    res.json(inserted);
    return;
  }
...
router.get("/scanner/score/:barcode", requireAuth, async (req, res) => {
  ...
  if (result.type === "general") {
    const score = calculateGeneralScore(result.product, excludedIngredients);
    ...
    await db.insert(scannedProductsTable).values({
      ...
      brand: result.product.brand,
      imageUrl: result.product.imageUrl,
      ingredients: result.product.ingredients || result.product.description,
      nutriments: {},
      labels: [],
      productType: "general",
      scoreIngredients: score.ingredients,
